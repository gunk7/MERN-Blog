...
const BlogSkeleton = () => (
  <div className="flex flex-col gap-6 max-w-3xl">
    ...
    <div className="flex gap-3">
      {[1, 2, 3].map((i) => (
        <Bone key={i} className="h-16 flex-1" />
      ))}
    </div>
    {[1, 2, 3, 4, 5].map((i) => (
...
const StatusBadge = ({ status }) => {
  const map = {
    published: {
      cls: "status-badge status-verified",
      icon: <CheckCircle size={11} />,
    },
    draft: {
      cls: "status-badge bg-surface-high text-on-surface-variant border border-surface-highest",
      icon: <Clock size={11} />,
    },
    scheduled: {
      cls: "status-badge bg-primary-fixed text-primary border border-primary-fixed-dim",
    ...
    <div className="relative">
      ...
      {open && (
        <>
          ...
          <div
            className="absolute left-0 top-full mt-2 z-20 bg-white border border-black/5
                          rounded-2xl shadow-lavender py-1 min-w-36 overflow-hidden"
          >
            {options.map((o) => (
              <button
                key={o}
                ...
                className="w-full text-left px-4 py-2.5 text-xs font-bold
                           text-on-surface-variant hover:bg-surface-low transition-colors capitalize"
              >
                → Move to {o}
              </button>
            ))}
...
const AdminBlogViewer = () => {
  ...
  const handleDelete = async () => {
    const result = await confirmAction(
      "Delete this post?",
      "This will soft-delete the post. Pass ?hard=true to permanently remove it.",
      "warning",
      "Yes, delete it!",
    );
    if (!result.isConfirmed) return;
    dispatch(deleteBlogAdmin({ blogId: blog._id }))
      .unwrap()
      .then(() => {
          ...
          <div className="flex flex-wrap gap-x-5 gap-y-2 pt-3 border-t border-surface-highest/40 text-xs text-on-surface-variant">
            ...
            {blog.scheduledFor && blog.status === "scheduled" && (
              <span className="flex items-center gap-1 text-primary font-bold">
                <TrendingUp size={11} /> Scheduled for{" "}
                {fmtDate(blog.scheduledFor)}
              </span>
            )}
            <span className="flex items-center gap-1">
            ...
            {blog.deletedAt && (
              <span className="flex items-center gap-1 text-red-500 font-bold">
                Deleted {fmtDate(blog.deletedAt)}
              </span>
            )}
          </div>
              ...
              {blog.tags.map((tag) => (
                <span
                  ...
                  className="flex items-center gap-1 text-[10px] font-bold uppercase tracking-widest
                             text-on-surface-variant bg-surface-low px-3 py-1 rounded-full border border-surface-highest/60"
                >
                  <Tag size={9} /> {tag}
                </span>
              ))}
